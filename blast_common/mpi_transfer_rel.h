#ifndef mpi_transfer_rel_h
#define mpi_transfer_rel_h
#ifdef USE_MPI
/**
   @file
   @brief Holds data about a protein pair, using the struct named rel.
...
struct mpi_transfer_rel {
  ...
  /**
     @brief Build a datatype representing the rel class.
  ...
  **/
  static void mpi_build_datatype(MPI_Datatype &mpi_type_info) {
    ...
    MPI_Datatype old_types[cnt_types] = {MPI_UNSIGNED, MPI_FLOAT};
  ...
  /**
     @brief Reserves- and initiates a list of type mpi_transfer_rel.
     @return An initiated list of objects of type mpi_transfer_rel.
  **/
  static mpi_transfer_rel *init(uint length) {
    mpi_transfer_rel *list = new mpi_transfer_rel[length];
    log_builder::test_memory_condition_and_if_not_abort(list!=NULL, __LINE__, __FILE__, __FUNCTION__);
    for(uint i = 0; i < length; i++) {list[i]=mpi_transfer_rel();}
    return list;
  }

  //! @return true if they are equal:
  static bool is_equal(struct mpi_transfer_rel &obj1, struct mpi_transfer_rel &obj2) {
    bool all_was_equal = true;    
    if(obj1.ind_out != obj2.ind_out)          all_was_equal = false;
    else if(obj1.distance != obj2.distance)   all_was_equal = false;
    return all_was_equal;
  }
  //! Enlarges the given list to a double size:
  static void enlarge(mpi_transfer_rel *&list, uint &length) {
    assert(length); // According to our expectations.
    assert(list);
    //! Reserves length
    const uint new_length = 2*length;
    mpi_transfer_rel *new_list = new mpi_transfer_rel[new_length];
    log_builder::test_memory_condition_and_if_not_abort(new_list!=NULL, __LINE__, __FILE__, __FUNCTION__);
    //! Ensures consistency:
    for(uint i = length; i < new_length; i++) {list[i]=mpi_transfer_rel();}
    memcpy(new_list, list, sizeof(mpi_transfer_rel)*length);
#ifndef NDEBUG
    //! Ensures that the list is correctly copied; when included handy for inspections:
    for(uint i = 0; i < length; i++) {
      assert(is_equal(list[i], new_list[i]));
    }
    mpi_transfer_rel empty = mpi_transfer_rel(); // Builds an empty object
  ...
  static void close(mpi_transfer_rel *&list, uint &length) {
    if(list) {
  ...
  //! The constructor:
  mpi_transfer_rel() : ind_out(0), distance(0.0)
  {};
  //! The constructor:
  mpi_transfer_rel(uint _out, float _dist) : ind_out(_out), distance(_dist) {}
};
...
/**
   ...
   @author Ole Kristian Ekseth (oekseth)
**/
typedef struct mpi_transfer_rel mpi_transfer_rel_t;
