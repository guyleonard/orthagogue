...
void cmd_list::set_arguments(char **array, int array_size, bool create_man_page) {
  ...
  if (create_man_page || (array_size <2) || FILE_INPUT_NAME == NULL)  { // file name not given or zero arguments given. PRints default message:   
    if(array_size < 2) fprintf(stdout, "!!\tBlast file not specified. The optional input parameters for %s, are:\n", PROGRAM_NAME);
    if(array_size < 2)    print_arguments(stdout, file_man_page);
    if(array_size < 2)   fprintf(stdout, "The software was developed by O.K. Ekseth under supervison of Dr. V.N. Mironov. Questions to be forwarded to %s\n", DEVELOPER_EMAIL);
